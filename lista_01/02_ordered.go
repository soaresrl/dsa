...
import (
	"fmt"
	"github.com/soaresrl/dsa/pkg/ordered_linked_list"
)

func main() {
	lst := ordered_linked_list.NewList()

	lst2 := ordered_linked_list.NewList()
	...
	fmt.Printf("AreEqual (lst, lst2) = %v\n", ordered_linked_list.AreEqual(lst, lst2))
	fmt.Printf("AreEqual (lst, lst) = %v\n", ordered_linked_list.AreEqual(lst, lst))
	fmt.Println(">>         End        <<")
